...
To show this issue, we'll use the following model, predicting the body fat
percentage of student athletes using fixed effects for their height and sex and
a random effect for their sport:

```{r PrepFat, message=FALSE, warning=FALSE}
...
```{r ShowFat, message=FALSE, warning=FALSE}
ais[1,]
...
predictInterval(ifmodel, ais[1,], include.resid.var=0, ignore.fixed.terms = 1) #predict the average body fat for a group of 196cm female baseball players, taking the global intercept (mean body fat) as fully known
predictInterval(ifmodel, ais[1,], include.resid.var=0, ignore.fixed.terms = "(Intercept)") #Same as above
predictInterval(ifmodel, ais[1,], include.resid.var=0, ignore.fixed.terms = 1:2) #as above, taking the first two fixed effects (intercept and height effect) as fully known
```
...
$$\hat\sigma^2_\mu'=\hat\sigma^2_\mu-\Sigma_{levels}\frac{1}{\Sigma_{groups(level)}1/(\hat\sigma^2_{level}+sigma^2_{group})}$$
...
```{r ShowFat2, message=FALSE, warning=FALSE}
predictInterval(ifmodel, ais[1,], include.resid.var=0, fix.intercept.variance = T) #predict the average body fat for a group of 196cm female baseball players, using an ad-hoc correction for the covariance of the intercept with the random intercept effects.
```
