...
In a multilevel model, a predictor measured at the individual level can
also be aggregated to the group level, and the two can carry *different*
meanings. The classic example is socio-economic status (SES): a
student’s own SES captures a *within-school* relationship, while the
average SES of a student’s school captures a *contextual* (or
*compositional*) effect – the additional influence of attending a more
or less advantaged school, over and above a student’s own background.

This vignette reproduces the kind of analysis described in the
`modelbased` package’s article [“Studying some effects in
context”](https://easystats.github.io/modelbased/articles/practical_context_effect.html)
using the tools in `merTools`. The data and model follow the
contextual-effects example that has long been used to teach multilevel
modeling (Raudenbush and Bryk, 2002; Gelman and Hill, 2007).

## The data

`merTools` ships with `hsb`, a subset of the 1982 *High School and
Beyond* survey: 7,185 students nested in 160 schools. It contains both a
student-level SES measure (`ses`) and the school-mean SES (`meanses`) –
...
We model math achievement as a function of a student’s own SES and the
average SES of their school, allowing each school its own intercept:
...
[`merTools::FEsim()`](https://jknowles.github.io/merTools/reference/FEsim.md)
summarizes the uncertainty in both, and
[`plotFEsim()`](https://jknowles.github.io/merTools/reference/plotFEsim.md)
highlights the terms whose interval excludes zero:
...
Because `meanses` enters the model directly, we can ask how the
predicted math score changes as we move a student across schools of
differing average SES while holding their own SES fixed.
[`wiggle()`](https://jknowles.github.io/merTools/reference/wiggle.md)
builds the counterfactual data and
[`predictInterval()`](https://jknowles.github.io/merTools/reference/predictInterval.md)
attaches prediction intervals:
...
summarizes how much being in a higher- versus lower-performing school
shifts the predicted outcome, binning schools by their expected rank.
[`plotREimpact()`](https://jknowles.github.io/merTools/reference/plotREimpact.md)
(new in this release) plots the result, and can overlay several cases
for comparison:
...
Lüdecke, D., et al. *modelbased: Studying some effects in context.*
easystats.
