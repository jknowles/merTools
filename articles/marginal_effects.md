...
One of the most common questions about multilevel models is how much
...
`newdata`. Each observation in `newdata` is identified by a `case`
number, because the function repeats each observation by the number of
random effect terms and number of breaks to simulate each term over.
Then the `grouping_var`

## Summarizing

## Plotting

Finally - you can plot the results marginalization to evaluate the
