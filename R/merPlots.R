...
#' @param scale a factor to multiply the effect standard deviation by, e.g. 1.96 for
...
#' @export
#' @import ggplot2
plotFEsim <- function(data, scale, var = "median", sd = "sd",
                      sigmaScale = NULL, oddsRatio = FALSE){
  if(!missing(sigmaScale)){
  ...
  ggplot(aes_string(x = xvar, y = var, ymax = "ymax",
                    ymin = "ymin"),
         dataa = data) + geom_point(size=I(3)) +
