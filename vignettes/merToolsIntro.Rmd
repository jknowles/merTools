...
```{r}
example2 <- wiggleObs(example1, var = "lectage", 
          values = c("1", "2", "3", "4", "5", "6"))

example2
```

The function `wiggleObs` allows us to create a new dataframe with copies of the 
variable that modify just one value. Chaining together `wiggleObs` calls, we 
...
```{r}
example3 <- wiggleObs(example1, var = "service", 
          values = c("0", "1"))
...
```{r}
REquantile(m1, quantile = 0.25, group = "s")
REquantile(m1, quantile = 0.25, group = "d")
```

Here we can see that group level "2518" corresponds to the 25th percentile 
of the effect for the student groups, and level "18" corresponds to the 25th 
...
```{r}
example4 <- averageObs(m1)
example4 <- wiggleObs(example4, var = "s", 
...
example5 <- averageObs(m1, varList = subExample)
example5
```
...
example6 <- averageObs(m2, varList = list("id" = "149"))
example6$btype <- "scold"
example6$situ <- "self"


tempdf <- wiggleObs(example6, var = "Gender", c("M", "F"))
tempdf <- wiggleObs(tempdf, var = "item", 
                    unique(VerbAgg$item))
